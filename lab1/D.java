...
class HttpServer {
    ...
    private void process(Request request, Response response) throws IOException {
        ...
        ArrayList<String> parsedUri = parseUri((request.getUri().equals("/") ? "/index.html" : request.getUri()));
 
        for (int i = 0; i < parsedUri.size(); i++) {
            parsedUri.set(i, URLDecoder.decode(parsedUri.get(i), "UTF-8"));
        }
 
        if (parsedUri.size() == 1) {
            if (parsedUri.get(0).equals("images") || parsedUri.get(0).equals("image")) {
                parsedUri.set(0, "images");
                parsedUri.add("index.html");
            }
        }
 
        String finalUri = "";
        for (String currentSection : parsedUri) {
            finalUri += "/" + currentSection;
        }
 
        File file = new File(root, finalUri);
        ...
        if (file.isFile()) {
            ...
            String responseETag = Integer.toString(Arrays.hashCode(body));
            String ifNoneMatchETag = request.getHeader("If-None-Match");
 
            response.setHeader("ETag", responseETag);
 
            if (ifNoneMatchETag != null && ifNoneMatchETag.equals(responseETag)) {
                response.setHeader("ETag", responseETag);
