...
public class FrontServlet extends HttpServlet {
    ...
    private Method findMethod(Class<?> pageClass, String methodName) {
        Method method = null;
        ...
        for (Class<?> clazz = pageClass; method == null && clazz != null; clazz = clazz.getSuperclass()) {
            for (Method currentMethod : clazz.getDeclaredMethods()) {
                if (currentMethod.getName().equals(methodName)) {
                    try {
                        method = clazz.getDeclaredMethod(methodName, currentMethod.getParameterTypes());
    ...
    private void process(Route route,
        ...
        Method beforeMethod = findMethod(pageClass, "before");
        Method afterMethod = findMethod(pageClass, "after");
        Method method = findMethod(pageClass, route.getAction());

        if (beforeMethod == null) {
            throw new ServletException("Cannot find 'before' method in class '" + pageClass.getSimpleName() + "'");
        }

        if (afterMethod == null) {
            throw new ServletException("Cannot find 'after' method in class '" + pageClass.getSimpleName() + "'");
        }
        ...
        try {
            ...
            throw new ServletException("Cannot invoke 'before' method in class '" + pageClass.getName() + "'", e);
        }
        ...
        try {
            ...
            for (Class<?> parameterType : method.getParameterTypes()) {
                ...
                if (parameterType == Map.class) {
                    parameters[parametersIndex++] = view;
                }
            }
        ...
        try {
            ...
            throw new ServletException("Cannot invoke 'after' method in class '" + pageClass.getName() + "'", e);
        }
